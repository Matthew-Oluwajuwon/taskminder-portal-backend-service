...
const auth = (request, response, next) => {
  if (!request.header("Authorization")) return response.status(401).send({
    responseCode: ResponseCode.UNAUTHORIZED,
    responseMessage: "Authenticate request with a valid token passed in the request header, concatenated with Bearer",
    data: null
  });
